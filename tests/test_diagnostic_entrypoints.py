...
def run_diagnostic(
    command: list[str], *, without_token: bool = False
...
def test_github_diagnostic_imports_in_both_invocation_forms_without_network(
    ...
    """Both entrypoints must reach configuration before requiring a token."""
    result = run_diagnostic(command, without_token=True)
    combined_output = result.stdout + result.stderr

    assert result.returncode == 1
    assert "[ERROR] GITHUB_TOKEN is missing." in result.stdout
